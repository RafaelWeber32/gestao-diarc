...
// ==========================================
// COLOQUE AS SUAS CHAVES DO FIREBASE ABAIXO!
// ==========================================
...
const App = () => {
  ...
  // --- FIREBASE EFFECTS (Buscando dados em tempo real) ---
  useEffect(() => {
    const unsubObras = onSnapshot(collection(db, 'obras'), (snap) => setObras(snap.docs.map(d => d.data())), console.error);
    
    // Auto-carrega dados padrão se estiver vazio na nuvem
    const unsubDisciplinas = onSnapshot(collection(db, 'disciplinas'), (snap) => {
      if (snap.empty) {
        const defaults = [
        ...
        defaults.forEach(d => setDoc(doc(db, 'disciplinas', d.id), d));
      } else {
        setDisciplinas(snap.docs.map(d => d.data()));
      }
    }, console.error);

    const unsubStatus = onSnapshot(collection(db, 'statusList'), (snap) => {
      if (snap.empty) {
        const defaults = [
        ...
        defaults.forEach(d => setDoc(doc(db, 'statusList', d.id), d));
      } else {
        setStatusList(snap.docs.map(d => d.data()));
      }
    }, console.error);

    const unsubDemandas = onSnapshot(collection(db, 'demandas'), (snap) => setDemandas(snap.docs.map(d => d.data())), console.error);

    return () => { unsubObras(); unsubDisciplinas(); unsubStatus(); unsubDemandas(); };
  }, []);

  // --- HANDLERS (Salvando dados na nuvem) ---
  const handleAddDemanda = async (e) => {
    ...
    try {
      if (editingDemandaId) {
        const docRef = doc(db, 'demandas', editingDemandaId.toString());
        await updateDoc(docRef, newDemanda);
        setEditingDemandaId(null);
      } else {
        const newId = Date.now().toString();
        const docRef = doc(db, 'demandas', newId);
  ...
  const handleDeleteDemanda = async (id) => {
    try {
      await deleteDoc(doc(db, 'demandas', id.toString()));
    } catch (error) {
  ...
  const handleAddObra = async (e) => {
    ...
    try {
      if (editingObraId) {
        const docRef = doc(db, 'obras', editingObraId.toString());
        await updateDoc(docRef, newObra);
        setEditingObraId(null);
      } else {
        const newId = Date.now().toString();
        await setDoc(doc(db, 'obras', newId), { ...newObra, id: newId });
      }
  ...
  const handleAddDisciplina = async (e) => {
    ...
    try {
      const newId = Date.now().toString();
      await setDoc(doc(db, 'disciplinas', newId), { id: newId, name: newDisciplina });
      setNewDisciplina('');
    } catch (error) {
      console.error(error);
    }
  ...
  const handleAddStatus = async (e) => {
    ...
    try {
      const newId = Date.now().toString();
      await setDoc(doc(db, 'statusList', newId), { ...newStatus, id: newId });
                ...
                <div className="p-4 flex-1">
                  ...
                  <div className="space-y-3">
                    {obra.demandasPendentes.length === 0 ? (
                      <p className="text-sm text-slate-400 italic">Nenhuma demanda pendente para esta obra.</p>
                    ) : (
    ...
    <div className="flex h-screen bg-slate-50 font-sans">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-10 pt-20 md:pt-10">
        <div className="max-w-6xl mx-auto">
          ...
          {activeTab === 'demandas' && <DemandasList />}
          {activeTab === 'nova_demanda' && <NovaDemandaForm />}
